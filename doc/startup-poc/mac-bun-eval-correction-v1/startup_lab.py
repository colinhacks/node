...
WORKLOADS = {
    # A semicolon is a no-op program accepted by both Node and Bun.  An empty
    # argument makes Bun print its help text while exiting successfully.
    "eval-empty": Workload("eval-empty", ("-e", ";"), False, require_empty_stdout=True),
    "empty-cjs": Workload("empty-cjs", (str(FIXTURES / "empty.cjs"),), False, require_empty_stdout=True),
    "empty-esm": Workload("empty-esm", (str(FIXTURES / "empty.mjs"),), False, require_empty_stdout=True),
...
def run_once(runtime: str, workload: Workload, environment: dict[str, str], timeout: float) -> dict[str, Any]:
    ...
    try:
        ...
        if status == "ok" and workload.require_empty_stdout and stdout:
            status = "unexpected-stdout"
            error = f"workload required empty stdout but emitted {len(stdout)} bytes"
